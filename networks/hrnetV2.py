...
def hr_basic_block(x, n_feature, stride_size=(1, 1), shortcut=False, **kwargs):
    ...
    if shortcut:
        x = tf.keras.layers.Conv2D(n_feature, 1, strides=stride_size, use_bias=False, kernel_initializer="he_normal",
...
def hr_bottleneck_block(x, n_feature, stride_size=(1, 1), shortcut=False, expansion=4, **kwargs):
    ...
    out = tf.keras.layers.Conv2D(n_decode_filter, 1, use_bias=False, kernel_initializer="he_normal",
                                 bias_initializer="zeros")(x)
    out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
    out = tf.keras.layers.Activation(tf.keras.activations.relu)(out)
    out = tf.keras.layers.Conv2D(n_decode_filter, 3, strides=stride_size, padding="same", use_bias=False,
                                 kernel_initializer="he_normal", bias_initializer="zeros")(out)
    out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
    out = tf.keras.layers.Activation(tf.keras.activations.relu)(out)
    out = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
                                 bias_initializer="zeros")(out)
    out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
...
def hr_transition_block(x, n_feature, **kwargs):
    ...
    for index, _n_feature in enumerate(n_feature):
        if (index + 1) == len(n_feature):
            ...
            feature = x[index]
            stride_size = (1, 1)
        o = tf.keras.layers.Conv2D(_n_feature, 3, strides=stride_size, padding="same", use_bias=False,
                                   kernel_initializer="he_normal", bias_initializer="zeros")(feature)
...
def object_attention(feature, prob, n_feature=256, scale=1, **kwargs):
    ...
    query = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
    ...
    query = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
                                   bias_initializer="zeros")(query)
    query = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(query)
    query = tf.keras.layers.Activation(tf.keras.activations.relu)(query)
    query = tf.keras.layers.Reshape([-1, tf.keras.backend.int_shape(query)[-1]])(query)

    key = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
                                 bias_initializer="zeros")(proxy_context)
    ...
    key = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
                                 bias_initializer="zeros")(key)
    key = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(key)
    key = tf.keras.layers.Activation(tf.keras.activations.relu)(key)
    key = tf.keras.layers.Reshape([-1, tf.keras.backend.int_shape(key)[-1]])(key)
    key = tf.keras.layers.Permute([2, 1])(key)

    value = tf.keras.layers.Conv2D(n_feature, 1, use_bias=False, kernel_initializer="he_normal",
                                   bias_initializer="zeros")(proxy_context)
    ...
    context = tf.keras.layers.Conv2D(tf.keras.backend.int_shape(feature)[-1], 1, use_bias=False,
                                     kernel_initializer="he_normal", bias_initializer="zeros")(context)
...
def hrnet_v2(x, n_class=35, include_top=True, n_channel=48, n_module=[1, 4, 3], n_branch=[2, 3, 4], stage1_channel=64,
             stage1_module=1, ocr_feature_channel=512, ocr_attention_channel=256, ocr_dropout_rate=0.05, ocr_scale=1,
             mode="ocr", **kwargs):
    ...
    if not isinstance(x, list):
        # Stem
        out = tf.keras.layers.Conv2D(64, 3, strides=2, padding="same", use_bias=False, kernel_initializer="he_normal",
                                     bias_initializer="zeros")(x)
        out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
        out = tf.keras.layers.Activation(tf.keras.activations.relu)(out)
        out = tf.keras.layers.Conv2D(64, 3, strides=2, padding="same", use_bias=False, kernel_initializer="he_normal",
        ...
        for index in range(len(x)):
            o = tf.keras.layers.Conv2D(n_channel[index], 3, padding="same", use_bias=False,
    ...
    if mode != "clsf":
        ...
        for _o in out[1:]:
            ...
            o = tf.keras.layers.Conv2D(ch, 3, strides=2, padding="same", use_bias=False, kernel_initializer="he_normal",
                                       bias_initializer="zeros")(o)
            o = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(o)
            o = tf.keras.layers.Activation(tf.keras.activations.relu)(o)
    ...
    if include_top:
        if mode != "clsf":
            out_aux = tf.keras.layers.Conv2D(tf.keras.backend.int_shape(out)[-1], 1, use_bias=True,
                                             kernel_initializer="he_normal", bias_initializer="zeros")(out)
            ...
            out_aux = tf.keras.layers.Conv2D(n_class, 1, use_bias=True, kernel_initializer="he_normal", name="logits",
                                             **kwargs)(out_aux)
            if mode == "ocr":
                out = tf.keras.layers.Conv2D(ocr_feature_channel, 3, padding="same", use_bias=False,
                                             kernel_initializer="he_normal", bias_initializer="zeros")(out)
                out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
                out = tf.keras.layers.Activation(tf.keras.activations.relu)(out)
                out = ocr_module(out, out_aux, ocr_feature_channel, ocr_attention_channel, ocr_dropout_rate, ocr_scale,
                                 **kwargs)
                out = tf.keras.layers.Conv2D(n_class, 1, use_bias=True, kernel_initializer="he_normal",
                                             name="logits_ocr", **kwargs)(out)
            else:
                out = out_aux
        else:
            out = tf.keras.layers.Conv2D(2048, 1, use_bias=False, kernel_initializer="he_normal",
                                         bias_initializer="zeros")(out)
            out = tf.keras.layers.BatchNormalization(axis=-1, momentum=0.9, epsilon=1e-5)(out)
            out = tf.keras.layers.Activation(tf.keras.activations.relu)(out)
...
def hrnet18_v2(x, n_class=35, include_top=True, mode="ocr"):
    out = hrnet_v2(x, n_class, include_top, n_channel=[18, 36, 72, 144], n_module=[1, 4, 3], n_branch=[2, 3, 4],
                   stage1_channel=64, stage1_module=1, ocr_feature_channel=512, ocr_attention_channel=256,
                   ocr_dropout_rate=0.05, ocr_scale=1, mode=mode)
    return out


def hrnet32_v2(x, n_class=35, include_top=True, mode="ocr"):
    out = hrnet_v2(x, n_class, include_top, n_channel=[32, 64, 128, 256], n_module=[1, 4, 3], n_branch=[2, 3, 4],
                   stage1_channel=64, stage1_module=1, ocr_feature_channel=512, ocr_attention_channel=256,
                   ocr_dropout_rate=0.05, ocr_scale=1, mode=mode)
    return out
...
def hrnet48_v2(x, n_class=35, include_top=True, mode="ocr"):
    out = hrnet_v2(x, n_class, include_top, n_channel=[48, 94, 192, 384], n_module=[1, 4, 3],
                   n_branch=[2, 3, 4], stage1_channel=64, stage1_module=1, ocr_feature_channel=512,
                   ocr_attention_channel=256, ocr_dropout_rate=0.05, ocr_scale=1, mode=mode)
    return out